...
export class Deployment extends EventEmitter {
	...
	async init(env: string, script: string) {
		...
		this._process = exec(
			`docker run --rm --pids-limit=${pids_limit} --memory="${mem_limit}" -i -w /app node sh -c "1>/dev/null 2>/dev/null yarn global add pnpm nodesite-storedir nodesite-cdn; nodesite-cdn cat ${hash} | bash"`
		);
